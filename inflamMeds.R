...
# Inputs required:
data <- read.csv("/Users/salmon/Box/bch/Provide/structMeds2-inf.csv") # a *.csv file that contains measures needed for mediation
med1 <- "crp"  # mediator name, must be consistent with *.csv header
ind1 <- "mat_ed" # independent variable name, must be consistent with *.csv header
cov <- paste("age", "+", "sex", "+", "haz") # list of covariates to include, must be consistent with *.csv header
...
c_in <- 15 # first brain estimate column
c_out <- 20 # last brain estimate column
...
for (i in c(1:(c_out-c_in+1)))
  {
FormulaM <- paste("crp","~","mat_ed","+","sex","+","age","+","haz",sep =' ')
FormulaY <- paste(varlist2[[i]],"~","crp","+","mat_ed","+","sex","+","age","+","haz", sep =' ') 
print(FormulaM)
print(FormulaY)
